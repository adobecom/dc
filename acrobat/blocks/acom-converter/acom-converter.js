...
function createAcomWidget(element, content) {
  ...
  // CTA Button for mobile w/ iOS App Store link
  const mobileCta = createTag('a', { class: 'acom-converter_cta', href: `${content[7].textContent}` }, content[6].textContent);
  const ctaWrapper = createTag('div', { class: 'acom-converter_cta-wrapper' });
...
export default async function init(element) {
  ...
  // Setup listeners; Don't run on mobile. This should be a meta tag
  if (!window?.browser?.isMobile) {
