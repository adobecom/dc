...
async function showModal() {
  ...
  mobileAnalyticsShown(aaVerbName, 'acrobat-exit-modal:shown');
}

// Check if `?showmodal=true` is in the URL and Remove Modal Dismissal
async function shouldForceShowModal() {
...
document.addEventListener('visibilitychange', () => {
  if (document.visibilityState === 'hidden' && !sessionStorage.getItem('modalDismissed')) {
    showModal();
  }
});
...
window.addEventListener('beforeunload', async (event) => {
  ...
  if (!shouldOpenModal(lastClickedLink)) {
    return;
  }
